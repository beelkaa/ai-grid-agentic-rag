...
function App() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: 'Hello. I am the AI GRID Assistant. How can I help you explore the documentation?' },
  ])
  ...
  async function sendQuestion(event, retryQuestion = question) {
    ...
    try {
      ...
      if (error.name !== 'AbortError') {
        setMessages((currentMessages) => [...currentMessages, { role: 'assistant', error: true, content: `I could not complete that request. ${error.message}` }])
      }
    } finally {
  ...
  function stopQuestion() {
    ...
    if (interruptedQuestion) {
      setMessages((currentMessages) => [...currentMessages, { role: 'assistant', interrupted: true, question: interruptedQuestion, complete: true, content: 'You interrupted the response. Try again when you are ready.' }])
    }
    setIsThinking(false)
  }
  ...
  return <main className="widget-stage">
    {isOpen ? <section className={`assistant-window${isFullscreen ? ' fullscreen' : ''}`} aria-label="AI GRID Assistant">
      <header className="window-header">
        <div className="identity"><BrandMark /><div><h1>AI GRID Assistant</h1><p><span className="status-dot" /> Ready when you are</p></div></div>
        <div className="window-controls"><button type="button" className="control-button" onClick={() => setIsFullscreen((current) => !current)} aria-label={isFullscreen ? 'Exit full view' : 'Open full view'} title={isFullscreen ? 'Exit full view' : 'Full view'}><span className={`fullscreen-glyph${isFullscreen ? ' exit' : ''}`} /></button><button type="button" className="control-button close" onClick={() => setIsOpen(false)} aria-label="Minimize assistant" title="Minimize">−</button></div>
      </header>
      ...
      <form className="composer" onSubmit={sendQuestion}>
        <textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={handleKeyDown} placeholder="Ask AI GRID Assistant..." rows="1" aria-label="Ask AI GRID Assistant" />
        {isThinking ? <button className="composer-action stop" type="button" onClick={stopQuestion} aria-label="Stop response" title="Stop response"><span /></button> : <button className="composer-action send" type="submit" disabled={!question.trim()} aria-label="Send message" title="Send message">↑</button>}
        <div className="composer-note"><span>Based on AI GRID documentation, AI may make mistakes.</span><kbd>Enter</kbd></div>
      </form>
    </section> : <button className="launcher" type="button" onClick={() => setIsOpen(true)}><BrandMark small /><span>Ask AI GRID Assistant</span><b>↑</b></button>}
  </main>
}
